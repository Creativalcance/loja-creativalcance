      ...
      <Link href={`/produto/${result.slug}`} className="group block">
        ...
        <div className="p-5">
          ...
          <div className="mt-2 flex items-start text-xs font-medium text-neutral-600">
            ...
            <span>
              Entrega estimada: {new Intl.DateTimeFormat("pt-PT").format(new Date(`${result.estimatedDeliveryDate}T12:00:00Z`))}
              <span className="mt-0.5 block font-normal text-neutral-400">
                {result.deliveryIncludesPersonalization ? "Considera a personalização mais rápida disponível" : "Produto sem personalização incluída"}
              </span>
              <span className="mt-1.5 block font-normal leading-4 text-neutral-500">
                Data meramente estimativa, dependente da aprovação da maquete, do tipo de personalização selecionado e do tempo de transporte.
              </span>
            </span>
          </div>
