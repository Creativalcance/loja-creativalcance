...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const config = getInstitutionalPage("metodologia-editorial", locale);
  return config ? {
      title: config.title,
      description: config.description,
      alternates: { canonical: localizePath("/metodologia-editorial", locale) },
      openGraph: {
        type: "website",
        locale: SITE_LOCALES[locale].htmlLang.replace("-", "_"),
        title: config.title,
        description: config.description,
        url: localizePath("/metodologia-editorial", locale),
      },
    }
...
export default async function Page() {
  ...
  const structuredData = buildEditorialStructuredData({
    name: config.h1,
    description: config.description,
    path: localizePath("/metodologia-editorial", locale),
