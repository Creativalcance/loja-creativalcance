...
export async function deleteAdminOrderAction(formData:FormData):Promise<void>{
  ...
  const admin=createSupabaseAdminClient();const{data:order,error:findError}=await admin.from("orders").select("id,status,order_number").eq("id",orderId).is("deleted_at",null).maybeSingle<{id:string;status:string;order_number:string}>();if(findError||!order)throw new Error(findError?.message??"Encomenda não encontrada.");
