...
export async function saveCheckoutShippingAction(
  ...
  try {
    ...
    if (!user) {
      redirectUrl = `${localizePath("/login", locale)}?next=${encodeURIComponent(localizePath("/checkout/expedicao", locale))}`;
    } else {
      ...
      redirectUrl = localizePath("/checkout/pagamento", locale);
    }
  } catch (error) {
    return {
      success: false,
