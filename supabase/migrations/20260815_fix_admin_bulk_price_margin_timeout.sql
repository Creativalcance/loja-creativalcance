...
begin
  ...
  if target_type = 'products' then
    ...
    set
      ...
      final_price = round(
        supplier_price / (1 - target_margin_percentage / 100),
        2
      ),
...
update public.bulk_price_change_batches as batch
set
  status = 'reverted',
  reverted_at = coalesce(batch.reverted_at, now())
where batch.status = 'applied'
  and batch.affected_rows = 0
  and not exists (
    select 1
    from public.bulk_price_change_items as item
